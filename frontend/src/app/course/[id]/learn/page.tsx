...
export default function LearnPage() {
	...
	function renderTask(task: PublicTaskDTO) {
		// Basic renderer per task type using public configuration
		const type = task.task_type;
		const cfg: any = task.configuration as any;
		return (
			<Card className="border-slate-800 bg-slate-900">
				...
				<CardContent className="space-y-4 text-slate-200">
					...
					{type === "SingleChoice" && Array.isArray(cfg?.options) ? (
					...
					{type === "MultipleChoice" && Array.isArray(cfg?.options) ? (
						<div className="space-y-2">
							{cfg.options.map((opt: string, idx: number) => (
								<div key={idx} className="flex items-center gap-2">
									<div className="h-3 w-3 rounded-sm border border-slate-600" />
									<div>{opt}</div>
								</div>
							))}
						</div>
					...
					{type === "ShortText" ? (
						<div className="rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-slate-200">
							{t("short_text")}
						</div>
					) : null}
					{type === "LongText" ? (
						<div className="min-h-24 rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-slate-200">
							{t("long_text")}
						</div>
					) : null}
					{type === "Ordering" && Array.isArray(cfg?.items) ? (
						<div className="space-y-1">
							{cfg.items.map((it: string, i: number) => (
								<div
									key={i}
					...
					{type === "FileUpload" ? (
						<div className="text-slate-300">{t("file_upload")}</div>
					) : null}
					{type === "CTFd" ? <div className="text-slate-300">CTFd</div> : null}
				</CardContent>
			</Card>
		);
	}
