...
import type { PublicTaskDTO, TaskDTO } from "@/api/tasks";
import { deleteTask, getTaskAdminById } from "@/api/tasks";
...
export default function EditExamDialog({
	...
	// Full task currently being edited (fetched with answers), and the id being loaded.
	...
	const onEditTask = async (taskId: number) => {
		try {
			...
			// The exam entity only carries the public task; fetch the full one
			// (with answers) so the editor can prefill correctly.
			const full = await getTaskAdminById(taskId);
			setEditTask(full);
		} catch (e) {
			console.error(e);
			toast({
	...
	const onTaskUpdated = (updated: TaskDTO) => {
		setItems((prev) =>
			prev.map((it) =>
				it.type === "task" && it.task.id === updated.id
					? {
							...it,
							task: {
								...it.task,
										...
										<div className="flex flex-col items-end gap-2 sm:flex-row">
											...
											{it.type === "task" ? (
												<Button
													...
													size="icon"
													className="border-slate-700 bg-transparent text-slate-300 hover:bg-slate-800"
													onClick={() => onEditTask(it.task.id)}
													disabled={loadingTaskId !== null}
													title={t("edit") ?? "Edit"}
												>
													{loadingTaskId === it.task.id ? (
